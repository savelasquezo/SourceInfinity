...
#ifdef ENABLE_BUY_ITEMS_AND_YANG
		str_to_number(pItem->llPriceYang, data[col++]);
#endif
#endif
...
bool CClientManager::InitializeItemAttrTable()
{
	...
	snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear, pendant, costume_weapon, costume_body, costume_hair FROM item_attr%s ORDER BY apply",
			GetTablePostfix());
#else
	snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear, costume_weapon, costume_body, costume_hair FROM item_attr%s ORDER BY apply",
			GetTablePostfix());
#endif
...
bool CClientManager::InitializeItemRareTable()
{
	char query[4096];
	snprintf(query, sizeof(query),
			"SELECT apply, apply+0, prob, lv1, lv2, lv3, lv4, lv5, weapon, body, wrist, foots, neck, head, shield, ear, pendant, costume_weapon, costume_body, costume_hair FROM item_attr_rare%s ORDER BY apply",
			GetTablePostfix());
