...
void CClientManager::RESULT_LOGIN_BY_KEY(CPeer * peer, SQLMsg * msg)
{
	...
	{
		snprintf(szQuery, sizeof(szQuery), "SELECT id, name, job, level, alignment, prestige, reborn, st, ht, dx, iq, part_main, part_hair, part_sash, x, y, skill_group, change_name FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
	}
	else
	{
		snprintf(szQuery, sizeof(szQuery), "SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, part_sash, x, y, skill_group, change_name FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
	}
	...
	for (DWORD i = 0; i < mysql_num_rows(pRes); ++i)
	{
		...
		for (j = 0; j < PLAYER_PER_ACCOUNT; ++j)
		{
			if (pkTab->players[j].dwID == player_id)
			{
				...
				sys_log(0, "%s %lu %lu hair %u sash %u", pkTab->players[j].szName, pkTab->players[j].x, pkTab->players[j].y, pkTab->players[j].wHairPart, pkTab->players[j].wSashPart);
				break;
			}
		}
		/*
...
void CClientManager::RESULT_LOGIN(CPeer * peer, SQLMsg * msg)
{
	...
	if (info->account_index == 0)
	{
		...
		else
		{
			...
			if (g_stLocale == "gb2312")
			{
				snprintf(queryStr, sizeof(queryStr), "SELECT id, name, job, level, alignment, prestige, reborn, st, ht, dx, iq, part_main, part_hair, part_sash, x, y, skill_group, change_name FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
			}
			else
			{
				snprintf(queryStr, sizeof(queryStr), "SELECT id, name, job, level, playtime, st, ht, dx, iq, part_main, part_hair, part_sash, x, y, skill_group, change_name FROM player%s WHERE account_id=%u", GetTablePostfix(), info->pAccountTable->id);
			}
