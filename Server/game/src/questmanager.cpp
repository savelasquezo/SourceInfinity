...
//#endif
	...
	void CQuestManager::Kill(unsigned int pc, unsigned int npc, unsigned int pc2)
    {
        ...
        if ((pPC = GetPC(pc)))
        {
            ...
            /* [hyo] ¸÷ kill? A?? A?i?? ??¨´ ??C? ?oA¤»cC?               quest script?^ when 171.kill begin ... ??C AU?? ?IC??? ?????®? ?l¸®??u??µµ
               ??? returnC?Ao ??? ?? ??µµ ?oC?C?µµ? ???CO. (2011/07/21)
            */
            // call script
            if (npc > 0 && pc2 == 0)
	...
	void CQuestManager::SetEventFlag(const string& name, int value)
	{
		...
		else if (name == "EventMoonlight") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_MOONSDROP, value != 0 ? true : false);
			}
		else if (name == "EventCordracon") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_CORDRACON, value != 0 ? true : false);
			}
		else if (name == "EventHexagonal") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_HEXAGONAL, value != 0 ? true : false);
			}
		else if (name == "EventCardsOkey") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_CARDSOKEY, value != 0 ? true : false);
			}
		else if (name == "EventFishPuzle") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_FISHPUZLE, value != 0 ? true : false);
			}
		else if (name == "EventRonarkGld") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_RONARKGLD, value != 0 ? true : false);
			}
		else if (name == "EventOxEventQz") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_OXEVENTQZ, value != 0 ? true : false);
			}
		else if (name == "EventBudokanPv") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_BUDOKANPV, value != 0 ? true : false);
			}
		else if (name == "EventTournment") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_TOURNAMNT, value != 0 ? true : false);
			}
		else if (name == "EventTotalWars") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_TOTALWARS, value != 0 ? true : false);
			}
		else if (name == "EventKingdomsW") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_KINGDOMSW, value != 0 ? true : false);
			}
		else if (name == "EventChristmas") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_CHRISTMAS, value != 0 ? true : false);
			}
		else if (name == "EventEasterEgg") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_EASTEREGG, value != 0 ? true : false);
			}
		else if (name == "EventSummerWkN") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_SUMMERWKN, value != 0 ? true : false);
			}
		else if (name == "EventHalloween") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_HALLOWEEN, value != 0 ? true : false);
			}
		else if (name == "EventValentine") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_VALENTINE, value != 0 ? true : false);
			}
		else if (name == "EventTanakasHT") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_TANAKASHT, value != 0 ? true : false);
			}
		else if (name == "EventMetinesEV") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_METINESEV, value != 0 ? true : false);
			}
		else if (name == "EventBossEvent") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_BOSSEVENT, value != 0 ? true : false);
			}
		else if (name == "EventFishRankg") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_FISHRANKG, value != 0 ? true : false);
			}
		else if (name == "EventMiningMap") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_MININGMAP, value != 0 ? true : false);
			}
		else if (name == "EventExpDropYG") {
			CGameEventsManager::instance().SetActivateEvent(EVENT_EXPDROPYG, value != 0 ? true : false);
			}
...
#ifdef ENABLE_RONARK_SYSTEM
	void CQuestManager::RonarkSetQuestFlag(const string& name2, int value, int guildid)
	{
		const int& name1 = guildid;
		stringstream ss;
		ss << name1 <<endl;
		const string& newname1 = ss.str();
		const string& name = newname1 + name2;
		TPacketSetEventFlag p;
		strlcpy(p.szFlagName, name.c_str(), sizeof(p.szFlagName));
		p.lValue = value;
		db_clientdesc->DBPacket(HEADER_GD_SET_EVENT_FLAG, 0, &p, sizeof(TPacketSetEventFlag));
	...
	int	CQuestManager::RonarkGetQuestFlag(const string& name2, int guildid)
	{
		const int& name4 = guildid;
		stringstream ss4;
		ss4 << name4 <<endl;
		const string& newname4 = ss4.str();
		const string& name = newname4 + name2;
		map<string, int>::iterator it = m_mapEventFlag.find(name);
