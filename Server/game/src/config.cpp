...
int g_server_id = 0;
string g_strWebMallURL = "http://bestproduction.com/";
	...
	*log_db = '\0';
	...
	while (fgets(buf, 256, fp))
	{
		...
		TOKEN("test_server")
		{
			...
			printf("\033[32m-----------------------------------------------\n");
			printf("\033[32mBEST Production Game Versiyon: 40250 [YAKUP POLAT]\n");
			printf("\033[32m-----------------------------------------------\n");
			continue;
		}
		...
		TOKEN("auth_server")
		{
			...
			if (!strcasecmp(szIP, "master"))
				fprintf(stdout, "BEST PRODUCTION SOURCE VERSION : 40250 \n");
			else
			{
