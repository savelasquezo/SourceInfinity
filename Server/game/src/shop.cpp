...
void CShop::SetShopItems(TShopItemTable * pTable, BYTE bItemCount)
{
	...
	for (int i = 0; i < bItemCount; ++i)
	{
		...
		else
		{
			...
			item.llPriceYang = pTable->llPriceYang;
#endif
#endif
...
#ifdef ENABLE_BUY_ITEMS_AND_YANG
	long long llPriceYang = r_item.llPriceYang;
#endif
#endif
...
//#ifdef ENABLE_BUY_ITEMS_AND_YANG
	if (dwWItemVnum > 0 || llPriceYang > 0)
	{
		if ((ch->CountSpecifyItem(dwWItemVnum) < (int)dwWItemPrice) || (ch->GetGold() < llPriceYang))
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_ITEM;
		if (ch->GetGold() < llPriceYang)
			return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
	}
	if (llPriceYang > 0)
	{
...
//#else
//	else if (ch->GetGold() < r_item.price)
//	{
//		return SHOP_SUBHEADER_GC_NOT_ENOUGH_MONEY;
//	}
//	
...
#ifdef ENABLE_BUY_ITEMS_AND_YANG
		pack2.items[i].llPriceYang = item.llPriceYang;
#endif
#endif
...
#ifdef ENABLE_BUY_ITEMS_AND_YANG
	pack2.item.llPriceYang = m_itemVector[pos].llPriceYang;
#endif
#endif
...
void CShop::InsertInMysql(LPCHARACTER lpSeller, LPITEM item, BYTE bPos, DWORD dwItemPrice)
{
	...
	std::unique_ptr<SQLMsg> pmsg(DBManager::instance().DirectQuery(
		...
		"VALUES('%u', '%s', '%s', %u, %u, %d, %u, %d, %d, %d, %d, %u, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d, %d)",
		get_table_postfix(), dwSellerPID, c_szSellerName, c_szItemName, dwItemVnum, dwItemCount, iItemRefine, dwItemPrice, iItemLevel, bPos, iItemType, iItemSubtype, iItemAntiflag, transmutation, socket1, socket2, socket3, socket4, socket5, socket6, attr1type, attr1value, attr2type, attr2value, attr3type, attr3value, attr4type, attr4value, attr5type, attr5value, attr6type, attr6value, attr7type, attr7value, apply0type, apply0value, apply1type, apply1value, apply2type, apply2value, apply3type, apply3value, apply4type, apply4value, apply5type, apply5value, apply6type, apply6value, apply7type, apply7value));
