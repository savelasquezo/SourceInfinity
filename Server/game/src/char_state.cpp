...
void CHARACTER::StateBattle()
{
	...
	if (IsSummonMonster() && !IsDead() && !IsStun())
	{
		...
		if (m_pkMobData->m_table.dwVnum == EREBUS_FINAL_BOSS)
		{
			if (pParty->CountMemberByVnum(EREBUS_HEALER) < SUMMON_HEALER_COUNT)
			{
				MonsterLog("Summoning party healers!");
				// 모자라는 녀석을 불러내 채웁시다.
				int iPercent = 0;
				if (GetMaxHP() >= 0)
					iPercent = (GetHP() * 100) / GetMaxHP();

				DWORD dwVnum = EREBUS_HEALER;

				int RespawnCount = SUMMON_HEALER_COUNT - pParty->CountMemberByVnum(EREBUS_HEALER);

				if (iPercent <= 20 && GetMaxStamina() < 4)
				{
					if (RespawnCount >= 1)
					{
						SetMaxStamina(4);
						LPCHARACTER healer_1 = CHARACTER_MANAGER::instance().SpawnMobRange(dwVnum, GetMapIndex(), 776500, 1503700, 776500, 1503700, true, true);
						if (healer_1)
						{
							pParty->Join(healer_1->GetVID());
							pParty->Link(healer_1);
						}
					}
					if (RespawnCount >= 2)
					{
						LPCHARACTER healer_2 = CHARACTER_MANAGER::instance().SpawnMobRange(dwVnum, GetMapIndex(), 773100, 1503700, 773100, 1503700, true, true);
						if (healer_2)
						{
							pParty->Join(healer_2->GetVID());
							pParty->Link(healer_2);
						}
					}
					if (RespawnCount >= 3)
					{
						LPCHARACTER healer_3 = CHARACTER_MANAGER::instance().SpawnMobRange(dwVnum, GetMapIndex(), 773200, 1500400, 773200, 1500400, true, true);
						if (healer_3)
						{
							pParty->Join(healer_3->GetVID());
							pParty->Link(healer_3);
						}
					}
					if (RespawnCount == 4)
					{
						LPCHARACTER healer_4 = CHARACTER_MANAGER::instance().SpawnMobRange(dwVnum, GetMapIndex(), 776500, 1500400, 776500, 1500400, true, true);
						if (healer_4)
						{
							pParty->Join(healer_4->GetVID());
							pParty->Link(healer_4);
						}
					}
				}
				else if (iPercent <= 40 && GetMaxStamina() < 3)
				{
					if (RespawnCount >= 1)
					{
						SetMaxStamina(3);
				...
				else if (iPercent <= 60 && GetMaxStamina() < 2)
				{
					if (RespawnCount >= 1)
					{
						SetMaxStamina(2);
				...
				else if (iPercent <= 80 && GetMaxStamina() < 1)
				{
					if (RespawnCount >= 1)
					{
						SetMaxStamina(1);
	...
	if (HasMobSkill())
	{
		for (unsigned int iSkillIdx = 0; iSkillIdx < MOB_SKILL_MAX_NUM; ++iSkillIdx)
		{
			if (CanUseMobSkill(iSkillIdx))
			{
				...
				if (victim->IsPC() && victim->IsActivateSecurity())
					return;
				#endif
