...
int COfflineShop::Buy(LPCHARACTER ch, BYTE bPos, bool isSearchBuy /*= false*/)
{
	...
	//DBManager::instance().Query("INSERT INTO pazar_gecmis%s (name, alanname, vnum, gold, time) values('%s', '%s', %u, %u, now());", get_table_postfix(), ch->GetOfflineShopOwner()->GetName(), ch->GetName(), pItem->GetVnum(), dwPrice);
	r_item.status = 2;
	...
	if (pItem->GetVnum() >= 55701 && pItem->GetVnum() <= 55715)
	{
	...
	DBManager::instance().DirectQuery("UPDATE player SET cheque = %d WHERE name = '%s'", ch->GetCheque(), ch->GetName());
