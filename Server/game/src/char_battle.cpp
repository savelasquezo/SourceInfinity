...
bool CHARACTER::Attack(LPCHARACTER pkVictim, BYTE bType)
{
	...
	if (IsPC() && IsActivateSecurity())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GUVENLIK_KILIDI_ACIKKEN_ATAK_YAPAMASSIN."));
		return false;
	}

	if (pkVictim && pkVictim->IsPC() && pkVictim->IsActivateSecurity())
	{
		if(IsPC())
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GUVENLIK_KILIDI_ACIK_OLAN_KARAKTERE_ATAK_YAPAMASSIN."));
		return false;
	}
#endif

#ifdef BESTPRODUCTION_BOT_KONTROL_SYSTEM
	if ((pkVictim->IsStone()) || (pkVictim->IsMonster()))
	{
...
#ifdef ENABLE_FIX_BUG_POLYMORPH
	...
	{	
		ChatPacket(CHAT_TYPE_INFO, "[Bilgi] %s isimli oyuncu donusumdeyken atak yapamassiniz!", pkVictim->GetName());
		return false;
	}
#endif
		...
		if ((GetLevel() - pkAttacker->GetLevel()) >= -10)
		{
			if (pkAttacker->GetRealAlignment() < 0)
			{
				if (pkAttacker->IsEquipUniqueItem(UNIQUE_ITEM_FASTER_ALIGNMENT_UP_BY_KILL))
					pkAttacker->UpdateAlignment(55);
				else
					pkAttacker->UpdateAlignment(10);
			}
			else
				pkAttacker->UpdateAlignment(15);
		}
...
void CHARACTER::Dead(LPCHARACTER pkKiller, bool bImmediateDead)
{
	...
	if (pkKiller && pkKiller->IsPC() && this->GetRaceNum() == 6420)
	{
		if (pkKiller->GetLevel() >= 80)
		{
			...
			if (pkKiller->GetStrokeByVid(GetVID(), &value))
			{
				...
				pkKiller->SetGolgeSavascisiTime();
			}
		}
		else
		{
...
#ifdef ENABLE_RONARK_SYSTEM
	if (!IsPC() && pkKiller && pkKiller->IsPC() && GetRaceNum() == 6438 && GetMapIndex() == 500)
	{
		if (!pkKiller->GetGuild())
			return;

		if (pkKiller->GetGuild()->RonarkOneKingID != 0)
		{
			CGuild* GetGuildRonark = CGuildManager::instance().FindGuild(pkKiller->GetGuild()->RonarkOneKingID);
			char szNotice[512+1];
			snprintf(szNotice, sizeof(szNotice), "<Ronark> El Gremio %s fue el Ganador del Evento Ronark!", GetGuildRonark->GetName());
			BroadcastNotice(szNotice);
		}

		quest::CQuestManager& KillRonark = quest::CQuestManager::instance();
		CRonark::instance().RonarkClear(pkKiller);
		KillRonark.SetEventFlag("RonarkKill",1);

		std::map<std::string, int>::iterator it;
		for (it=RonarkAttackList.begin(); it!=RonarkAttackList.end(); ++it)
			KillRonark.RonarkSetQuestFlag("RonarkPoint",0 , (*it).second);
	}
#endif
...
#ifdef ENABLE_MULTI_RANKING
	if ((pkKiller && pkKiller->IsPC()) && this->IsPC())
	{
		const std::string ip1 = pkKiller->GetDesc()->GetHostName();
		const std::string ip2 = this->GetDesc()->GetHostName();
		if ((ip1.compare(ip2) != 0) && (this->GetLevel() >= 30))
		{
			IncreaseKills(pkKiller, this, INCREASE_KILLS_PLAYER);
			DBManager::instance().DirectQuery("UPDATE player SET rcrd_rkn1 = rcrd_rkn1+1 WHERE name = '%s'", pkKiller->GetName());
			DBManager::Instance().DirectQuery("INSERT INTO log.ranking_log (player_id, player_name, player_empire, player_killed_name, date) VALUES(%u, '%s', %u, '%s', NOW())", pkKiller->GetPlayerID(), pkKiller->GetName(), pkKiller->GetEmpire(), this->GetName());
			if (pkKiller && pkKiller->IsPC() && pkKiller->GetJob() == JOB_WARRIOR)
			{
				if (this->IsPC() && this->GetJob() == JOB_WARRIOR)
				{
					DBManager::instance().DirectQuery("UPDATE player SET rcrd_rkn2 = rcrd_rkn2+1 WHERE name = '%s'", pkKiller->GetName());
				}
			}
			if (pkKiller && pkKiller->IsPC() && pkKiller->GetJob() == JOB_ASSASSIN)
			{
				if (this->IsPC() && this->GetJob() == JOB_ASSASSIN)
				{
					DBManager::instance().DirectQuery("UPDATE player SET rcrd_rkn3 = rcrd_rkn3+1 WHERE name = '%s'", pkKiller->GetName());
				}
			}
			if (pkKiller && pkKiller->IsPC() && pkKiller->GetJob() == JOB_SURA)
			{
				if (this->IsPC() && this->GetJob() == JOB_SURA)
				{
					DBManager::instance().DirectQuery("UPDATE player SET rcrd_rkn4 = rcrd_rkn4+1 WHERE name = '%s'", pkKiller->GetName());
				}
			}
			if (pkKiller && pkKiller->IsPC() && pkKiller->GetJob() == JOB_SHAMAN)
			{
				if (this->IsPC() && this->GetJob() == JOB_SHAMAN)
				{
					DBManager::instance().DirectQuery("UPDATE player SET rcrd_rkn5 = rcrd_rkn5+1 WHERE name = '%s'", pkKiller->GetName());
				}
			}
#ifdef ENABLE_WOLFMAN_CHARACTER
			if (pkKiller && pkKiller->IsPC() && pkKiller->GetJob() == JOB_WOLFMAN)
			{
				if (this->IsPC() && this->GetJob() == JOB_WOLFMAN)
				{
					DBManager::instance().DirectQuery("UPDATE player SET rcrd_rkn6 = rcrd_rkn6+1 WHERE name = '%s'", pkKiller->GetName());
				}
			}
#endif
		}
...
bool CHARACTER::Damage(LPCHARACTER pAttacker, int dam, EDamageType type) // returns true if dead
{
	...
	switch (type)
	{
		...
		case DAMAGE_TYPE_NORMAL_RANGE:
			...
			dam = dam * (100 - MIN(99, GetPoint(POINT_RESIST_HUMAN))) / 100;
...
//		if (pAttacker)
//		{
//			if (GetVID() == pAttacker->GetQuestNPCID()){
//				if ((dam > pAttacker->GetQuestDamage()) && (type != DAMAGE_TYPE_POISON)) 
...
//		}
//#endif
		PointChange(POINT_HP, -abs(dam), false);
	}
...
#ifdef ENABLE_RONARK_SYSTEM
	if (GetRaceNum() == 6438 && GetMapIndex() == 500){
		...
		if (pAttacker->IsPC() && bReturn == true)
		{
			quest::CQuestManager& R = quest::CQuestManager::instance();
			int GetOneGuildID = R.GetEventFlag("OneGuildIDRanked");
			pAttacker->GetGuild()->RonarkOneKingID = GetOneGuildID;

			int newdam = dam;
			CRonark::instance().RonarkSet(pAttacker, newdam);
			RonarkAttackList.insert(std::pair<std::string,int>(pAttacker->GetGuild()->GetName(),pAttacker->GetGuild()->GetID()));
		}
	}
#endif
...
*/
	...
	if (to->GetNewPetSystem()) {
		if (to->GetNewPetSystem()->GetLevel() < 155)
		{
...
//	iArrowCount = MIN(iArrowCount, pkArrow->GetCount());
	...
	iArrowCount = MIN(iArrowCount, 200); //Rune LamdaSystem
	*ppkBow = pkBow;
	*ppkArrow = pkArrow;
	return iArrowCount;
}
	...
	void operator()(LPENTITY ent)
	{
		if (ent->IsType(ENTITY_CHARACTER))
		{
			...
			if (number(1, 100) <= 100) // 임시로 50% 확률로 적을 끌어온다
				if (DISTANCE_APPROX(ch->GetX() - m_ch->GetX(), ch->GetY() - m_ch->GetY()) < 7500) //Default 5000
					if (ch->CanBeginFight())
						ch->BeginFight(m_ch);
		}
	}
