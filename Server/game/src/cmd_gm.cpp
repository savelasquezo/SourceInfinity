...
ACMD(do_item)
{
	...
	if (item)
	{
		if (item->IsDragonSoul())
		{
			...
			else
			{
				...
				if (!ch->DragonSoul_IsQualified())
				{
					...
					ch->ChatPacket(CHAT_TYPE_INFO, "No hay suficiente espacio en el inventario.");
			}
		}
		...
		else if (item->IsStone())
		{
			...
			{
				M2_DESTROY_ITEM(item);
				ch->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("No hay suficiente espacio en el inventario."));		
			}
		}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "#%u elemento no existe por ese vnum.", dwVnum);
	}
}
...
ACMD(do_pets)
{
	...
	if (!*item || !*bonus)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "/pets <cod_item> <b1:v1,b2:v2,b3:v3,b4:v4,b5:v5>");
		return;
	}
	...
	if (strstr(bonus, ",") == NULL) {
		ch->ChatPacket(CHAT_TYPE_INFO, "1 bonus eklediniz.");
		int vnum=0;
		str_to_number(vnum, item);
		LPITEM i = ITEM_MANAGER::instance().CreateItem(vnum);
		if (!i) {
			ch->ChatPacket(CHAT_TYPE_INFO, "");
			ch->ChatPacket(CHAT_TYPE_INFO, "Bu oge mevcut degil.");
			return;
		}
		int bonus_vnum=0;
		int bonus_value=0;
		...
		i->SetForceAttributePets(0, bonus_vnum, bonus_value);
		i->SetSocket(0, 1);
		int inventario = ch->GetEmptyInventory(i->GetSize());
		if (inventario != -1) {
			i->AddToCharacter(ch, TItemPos(INVENTORY, inventario));
			ch->ChatPacket(CHAT_TYPE_INFO, "Basarili!");
		}else{
			M2_DESTROY_ITEM(i);
			ch->ChatPacket(CHAT_TYPE_INFO, "No hay suficiente espacio en su inventario.");
		}
	}else{
		std::vector<std::string> bonus1;
		split(std::string(bonus), ',', bonus1);
		if (bonus1.size()>0) {
			ch->ChatPacket(CHAT_TYPE_INFO, "%d bonus ekledin.", (int)bonus1.size());
			if (countChars(bonus, ':') != (int)bonus1.size()) {
				ch->ChatPacket(CHAT_TYPE_INFO, "");
				ch->ChatPacket(CHAT_TYPE_INFO, "Eksik, deger ve bonus arasinda.");
				return;				
			}
			int vnum=0;
			str_to_number(vnum, item);
			...
			for (int n=0; n<b_size; n++) {
				...
				str_to_number(bonus_vnum, std::string(bonus2[0]).c_str());
				str_to_number(bonus_value, std::string(bonus2[1]).c_str());
				i->SetForceAttributePets(n, bonus_vnum, bonus_value);
			}
			i->SetSocket(0, 1);
			int inventario = ch->GetEmptyInventory(i->GetSize());
			if (inventario != -1) {
				...
				M2_DESTROY_ITEM(i);
				ch->ChatPacket(CHAT_TYPE_INFO, "Envanterinizde yeterli yer yok.");
			}
		}
	}
}
...
ACMD(do_buy_marble)
{
	//if (ch->IsObserverMode() || ch->GetExchange() || ch->GetShop() || ch->GetOfflineShop())
	if(ch->IsObserverMode() || ch->GetExchange() || ch->GetMyShop() || ch->GetShopOwner() || ch->IsOpenSafebox() || ch->IsCubeOpen() || ch->IsCombOpen() || ch->GetOfflineShopOwner())
		return;
	...
	if (!*arg1 || !*arg2)
	{
		ch->ChatPacket(CHAT_TYPE_INFO,  LC_TEXT("Syntax: buy_marble_item <mobVnum> <itemCount>"));
		return;
	}
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO,  LC_TEXT("mobVnum not exist or is wrong."));
		return;
	}
	
	if (!itemCount)
	{
		ch->ChatPacket(CHAT_TYPE_INFO,  LC_TEXT("itemCount not exist."));
		return;
	}

	if (itemCount <= 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO,  LC_TEXT("itemCount bellow 0."));
		return;
	}
	
	if (itemCount > 10)
	{
		ch->ChatPacket(CHAT_TYPE_INFO,  LC_TEXT("itemCount higher than 10."));
		return;
	}
	...
	switch(mobVnum)
	{
		...
		default:
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "?g?? ama sat? almak istedi?niz D?淆? K?esi mevcut de?l.");
			price = 0;
			break;
		}
	}
	...
	if (price == 0)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Fiyat yanl詰.");
		return;
	}
	
	if (ch->GetGold() < price*itemCount)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Bu D?淆? K?esini almak i?n yeterli yang yok.");
		return;
	}
	...
	for (int createMarble = 0; createMarble < itemCount; createMarble++)
	{
		...
		if (item)
		{
			...
			{
				M2_DESTROY_ITEM(item);
				ch->ChatPacket(CHAT_TYPE_INFO, "Daha fazla D?淆? K?esi almak i?n yeterli stok alan?yok.");
				break;
			}
		}
		else
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "#%d bu D?淆? K?esi oyunumuzda mevcut de?l.", 70104);
			break;
		}
	}
}
...
ACMD (do_item_full_set)
{
	...
	switch (job)
	{
		case JOB_WARRIOR:
		{
			item = ITEM_MANAGER::instance().CreateItem(21609);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(8119);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53819);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53829);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53839);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53849);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53859);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53869);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53776);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(18099);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(85004);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(67630);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
		}
		break;
		...
		case JOB_ASSASSIN:
		{
			item = ITEM_MANAGER::instance().CreateItem(21619);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(8129);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53819);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
		...
		case JOB_SURA:
		{
			item = ITEM_MANAGER::instance().CreateItem(21629);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(8109);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53819);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
		...
		case JOB_SHAMAN:
		{
			item = ITEM_MANAGER::instance().CreateItem(21639);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(8149);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
				M2_DESTROY_ITEM(item);
			item = ITEM_MANAGER::instance().CreateItem(53819);
			if (!item || !item->EquipTo(ch, item->FindEquipCell(ch)))
...
ACMD (do_attr_full_set)
{
	...
	switch (job)
	{
		case JOB_WARRIOR:
		{
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_NORMAL_HIT_DAMAGE_BONUS, 500);
				item->SetForceAttribute(1, APPLY_SKILL_DAMAGE_BONUS, 500);
				item->SetForceAttribute(2, APPLY_PENETRATE_PCT, 500);
				item->SetForceAttribute(3, APPLY_CRITICAL_PCT, 500);
				item->SetForceAttribute(4, APPLY_CAST_SPEED, 500);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_RESIST_EARTH, 500);
				item->SetForceAttribute(1, APPLY_RESIST_WIND, 500);
				item->SetForceAttribute(2, APPLY_RESIST_FIRE, 500);
				item->SetForceAttribute(3, APPLY_ATT_SPEED, 100);
				item->SetForceAttribute(4, APPLY_MOV_SPEED, 100);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_RESIST_ELEC, 500);
				item->SetForceAttribute(1, APPLY_RESIST_DARK, 500);
				item->SetForceAttribute(2, APPLY_RESIST_ICE, 500);
				item->SetForceAttribute(3, APPLY_IMMUNE_STUN, 1);
				item->SetForceAttribute(4, APPLY_MAX_HP, 20000);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_ATTBONUS_EARTH, 500);
				item->SetForceAttribute(1, APPLY_ATTBONUS_WIND, 500);
				item->SetForceAttribute(2, APPLY_ATTBONUS_FIRE, 500);
				item->SetForceAttribute(3, APPLY_SKILL_DEFEND_BONUS, 500);
				item->SetForceAttribute(4, APPLY_MAX_HP, 20000);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_ATTBONUS_ELEC, 500);
				item->SetForceAttribute(1, APPLY_ATTBONUS_DARK, 500);
				item->SetForceAttribute(2, APPLY_ATTBONUS_ICE, 500);
				item->SetForceAttribute(3, APPLY_NORMAL_HIT_DEFEND_BONUS, 500);
				item->SetForceAttribute(4, APPLY_MAX_HP, 20000);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_CON, 500);
				item->SetForceAttribute(1, APPLY_INT, 500);
				item->SetForceAttribute(2, APPLY_ATTBONUS_STONE, 500);
				item->SetForceAttribute(3, APPLY_ATTBONUS_BOSS, 500);
				item->SetForceAttribute(4, APPLY_MAX_HP, 20000);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_DEX, 500);
				item->SetForceAttribute(1, APPLY_STR, 500);
				item->SetForceAttribute(2, APPLY_ATT_GRADE_BONUS, 500);
				item->SetForceAttribute(3, APPLY_MAGIC_ATT_GRADE, 500);
				item->SetForceAttribute(4, APPLY_MAX_HP, 20000);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_RESIST_SWORD, 500);
				item->SetForceAttribute(1, APPLY_RESIST_TWOHAND, 500);
				item->SetForceAttribute(2, APPLY_RESIST_DAGGER, 500);
				item->SetForceAttribute(3, APPLY_BLOCK, 500);
				item->SetForceAttribute(4, APPLY_MAX_HP, 20000);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_RESIST_BOW, 500);
				item->SetForceAttribute(1, APPLY_RESIST_BELL, 500);
				item->SetForceAttribute(2, APPLY_RESIST_FAN, 500);
				item->SetForceAttribute(3, APPLY_KILL_HP_RECOVER, 500);
				item->SetForceAttribute(4, APPLY_MAX_HP, 20000);
			}

			item = ch->GetWear(WEAR_ARROW);
			if (item != NULL)
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_OMNIATTBONUS, 500);
				item->SetForceAttribute(1, APPLY_OMNIRESIST, 500);
				item->SetForceAttribute(2, APPLY_RESIST_MAGIC, 500);
				item->SetForceAttribute(3, APPLY_RESIST_ASSASSIN, 500);
				item->SetForceAttribute(4, APPLY_MAX_HP, 20000);
			}
			...
			{
				item->ClearAttribute();
				item->SetForceAttribute(0, APPLY_RESIST_WARRIOR, 500);
				item->SetForceAttribute(1, APPLY_RESIST_SURA, 500);
				item->SetForceAttribute(2, APPLY_RESIST_SHAMAN, 500);
				item->SetForceAttribute(3, APPLY_MAX_HP_PCT, 500);
				item->SetForceAttribute(4, APPLY_MAX_HP, 20000);
			}
		}
		break;
		...
		case JOB_SURA:
		{
			...
			if (item != NULL)
			{
				...
				item->SetForceAttribute(1, APPLY_RESIST_WIND, 500);
				item->SetForceAttribute(2, APPLY_RESIST_FIRE, 500);
				item->SetForceAttribute(3, APPLY_ATT_SPEED, 200);
				item->SetForceAttribute(4, APPLY_MOV_SPEED, 200);
			}
