...
void Initialize()
{
		...
		do
		{
			...
			else
			{
				switch (iColCount++)
				{
					...
					case 8: str_to_number(fish_info[idx].prob[4], szCol); break;
					case 9: str_to_number(fish_info[idx].prob[5], szCol); break;
					case 10: str_to_number(fish_info[idx].prob[6], szCol); break;
					case 11: str_to_number(fish_info[idx].prob[7], szCol); break;
					case 12: str_to_number(fish_info[idx].prob[8], szCol); break;
					case 13: str_to_number(fish_info[idx].difficulty, szCol); break;
					case 14: str_to_number(fish_info[idx].time_type, szCol); break;
					case 15: str_to_number(fish_info[idx].length_range[0], szCol); break;
					case 16: str_to_number(fish_info[idx].length_range[1], szCol); break;
					case 17: str_to_number(fish_info[idx].length_range[2], szCol); break;
					case 18: // 0
					case 19: // 1
					case 20: // 2
	...
	{
		sys_log(0, "FISH: %-24s vnum %5lu prob %4d %4d %4d %4d %4d %4d %4d %4d %4d len %d %d %d",
				fish_info[i].name,
				fish_info[i].vnum,
				fish_info[i].prob[0],
				fish_info[i].prob[1],
...
int DetermineFish(LPCHARACTER ch)
{
	...
	int prob_idx = rod->GetValue(3);

	if (prob_idx < 0)
		return 0;

	int adjust = 0;
	if (quest::CQuestManager::instance().GetEventFlag("FishMissPct") != 0)
	{
		int fish_pct_value = MINMAX(0, quest::CQuestManager::instance().GetEventFlag("FishMissPct"), 200);
...
void FishingPractice(LPCHARACTER ch)
{
	...
	if (rod && rod->GetType() == ITEM_ROD)
	{
		...
		if ( rod->GetRefinedVnum() > 0 && rod->GetSocket(0) < rod->GetValue(2) && number(1,100) < rod->GetValue(1) )
		{
...
EVENTFUNC(fishing_event)
{
	...
	switch (info->step)
	{
		case 0:	// 흔들리기 또는 떡밥만 날아감
			...
			if (int(ch->GetQuestFlag("hapis.mod")) == 1)
			{
				int sans = number(1, 10);
				if (sans < 6)
				{
					info->fish_id = 27802;
				}
				else if (sans >= 6 && sans <= 8)
				{
					info->fish_id = 27803;
				}
				else
...
void Take(fishing_event_info* info, LPCHARACTER ch)
{
	if (!ch->IsNearWater())
	{
		LogManager::instance().HackLog("FISHING_NON_NEAR_WATER", ch);
		fishing::FishingFail(ch);
		return;
	}
	else if (!ch->GetWear(WEAR_WEAPON) || ch->GetWear(WEAR_WEAPON)->GetType() != ITEM_ROD)
	{
		LogManager::instance().HackLog("FISHING_ROD_HACK", ch);
		fishing::FishingFail(ch);
		return;
	}
...
int main(int argc, char **argv)
{
	...
	for (int i = 0; i < MAX_FISH; ++i)
	{
		printf("%s\t%u\t%u\t%u\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d\t%d",
				fish_info[i].name,
				fish_info[i].vnum,
