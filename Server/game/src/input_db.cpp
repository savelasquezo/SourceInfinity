...
void CInputDB::PlayerLoad(LPDESC d, const char * data)
{
...

	{
		...
		char buf[55];
		snprintf(buf, sizeof(buf), "%s %d %d %d %ld %d %d",
				inet_ntoa(ch->GetDesc()->GetAddr().sin_addr), ch->GetGold(), ch->GetCheque(), g_bChannel, ch->GetMapIndex(), ch->GetAlignment(), ch->GetReborn());
#else
		char buf[51];
		snprintf(buf, sizeof(buf), "%s %d %d %ld %d %d",
				inet_ntoa(ch->GetDesc()->GetAddr().sin_addr), ch->GetGold(), g_bChannel, ch->GetMapIndex(), ch->GetAlignment(), ch->GetReborn());
#endif
	...
	ch->QueryAccountLockSize();
	ch->QueryAccountLockPassword();
#endif
...
//DEBUG P2PSendSwitchbot: No switchbot found to transfer. 15/11/2020
#ifdef ENABLE_SWITCHBOT
				case SWITCHBOT:
#endif
