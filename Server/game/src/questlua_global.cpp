...
namespace quest
{
	...
	int _syschat_kirmizi(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cffff0000%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_mavi(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cff0000ff%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_turkuaz(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cff00C78C%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_mor(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cffDA70D6%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_yesil(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cff00ff00%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_altin(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cffffcc00%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_gri(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cff888888%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_beyaz(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cffffffff%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_sari(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cffffff00%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_turuncu(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cffFF4500%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_pembe(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "|cFFFFC0CB%s|r", s.str().c_str());
		return 0;
	}

	int _syschat_red(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFff0000|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_acik_mavi(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFF00FFFF|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_orange(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFFF4522|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_yellow(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFFFD700|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_light_blue(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFF87CEFA|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_light_green(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFF70E970|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_light_kahverengi(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFD2691E|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_dark_orange(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFFF8C00|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_aqua(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFF00FFFF|H|h", s.str().c_str());
	...
	int _syschat_crimson(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFDC143C|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_acik_mavi_bilgi(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFFFD700|H|h[BÝLGÝ] :|cFF00FFFF|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_acik_mavi_duyuru(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFFFD700|H|h[DUYURU] :|cFF00FFFF|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_acik_mavi_not(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFFFD700|H|h[NOT] :|cFF00FFFF|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_acik_mavi_oneri(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFFFD700|H|h[ÖNERÝ] :|cFF00FFFF|H|h", s.str().c_str());
		return 0;
	}

	int _syschat_acik_mavi_system(lua_State* L)
	{
		ostringstream s;
		combine_lua_string(L, s);
		CQuestManager::Instance().GetCurrentCharacterPtr()->ChatPacket(CHAT_TYPE_INFO, "%s %s", "|cFFFFD700|H|h[System] :|cFF00FFFF|H|h", s.str().c_str());
		return 0;
	}
	...
	void RegisterGlobalFunctionTable(lua_State* L)
	{
		...
		luaL_reg global_functions[] =
		{
			...
			{	"syschat_kirmizi",			_syschat_kirmizi		},
			{	"syschat_mavi",				_syschat_mavi			},
			{	"syschat_turkuaz",			_syschat_turkuaz		},
			{	"syschat_mor",				_syschat_mor			},
			{	"syschat_yesil",			_syschat_yesil			},
			{	"syschat_altin",			_syschat_altin			},
			{	"syschat_gri",				_syschat_gri			},
			{	"syschat_beyaz",			_syschat_beyaz			},
			{	"syschat_sari",				_syschat_sari			},
			{	"syschat_turuncu",			_syschat_turuncu		},
			{	"syschat_pembe",			_syschat_pembe			},

			{	"syschat_red",					_syschat_red					},
			{	"syschat_acik_mavi",			_syschat_acik_mavi				},
			{	"syschat_orange",				_syschat_orange					},
			{	"syschat_yellow",				_syschat_yellow					},
			{	"syschat_light_blue",			_syschat_light_blue				},
			{	"syschat_light_green",			_syschat_light_green			},
			{	"syschat_light_kahverengi",		_syschat_light_kahverengi		},
			{	"syschat_dark_orange",			_syschat_dark_orange			},
			{	"syschat_aqua",					_syschat_aqua					},
			{	"syschat_crimson",				_syschat_crimson				},
			{	"syschat_acik_mavi_bilgi",		_syschat_acik_mavi_bilgi		},
			{	"syschat_acik_mavi_duyuru",		_syschat_acik_mavi_duyuru		},
			{	"syschat_acik_mavi_not",		_syschat_acik_mavi_not			},
			{	"syschat_acik_mavi_oneri",		_syschat_acik_mavi_oneri		},
			{	"syschat_acik_mavi_system",		_syschat_acik_mavi_system		},
