...
namespace MeleyLair
{
	...
	int eggsSpawnPos[MOBCOUNT_RESPAWN_STONE_STEP2][2] =
	{
		{118, 142},
		{118, 119},
		{143, 119},
		...
		if ((pEventInfo->bStep == 2) || (pEventInfo->bStep == 3))
		{
			...
			if (stones_count == 0)
			{
				...
				if (dwTime >= pMap->GetLastStoneKilledTime())
				{
					...
					{
						DWORD dwRandom = number(3, 10);
						pMap->Spawn((DWORD)(MeleyLair::MOBVNUM_RESPAWN_STONE_STEP2), eggsSpawnPos[i][0], eggsSpawnPos[i][1], 0);
					}
	...
	bool CMgrMap::IsPartecipant(DWORD dwPlayerID)
	{
		...
		//
		//return bCheck;
		//STR FIX MELEYLAIR 16/04/2021
		itertype(v_Partecipants) myIter= std::find(v_Partecipants.begin(), v_Partecipants.end(), dwPlayerID);
		
		if (myIter != v_Partecipants.end())
			return true;
		
		return false;
		//END FIX MELEYLAIR 16/04/2021 
	}
	...
	void CMgrMap::Start()
	{
		if (!pkSectreeMap)
			...
			pkMainNPC = Spawn((DWORD)(NPC_VNUM), 135, 177, 8), pkGate = Spawn((DWORD)(GATE_VNUM), 129, 175, 5), pkBoss = Spawn((DWORD)(BOSS_VNUM), 130, 77, 1), pkStatue1 = Spawn((DWORD)(STATUE_VNUM), 123, 137, 5, true), pkStatue2 = Spawn((DWORD)(STATUE_VNUM), 123, 124, 5, true), pkStatue3 = Spawn((DWORD)(STATUE_VNUM), 137, 124, 5, true), pkStatue4 = Spawn((DWORD)(STATUE_VNUM), 137, 137, 5, true);
	}
	...
	void CMgr::Register(LPCHARACTER pkChar, int & iRes1, int & iRes2)
	{
		...
		if (!posSub.x)
		{
			...
			if (pkSectreeMap)
				SetSubXYZ(pkSectreeMap->m_setting.iBaseX + 1434 * 100, pkSectreeMap->m_setting.iBaseY + 1421 * 100, 0);
		}
