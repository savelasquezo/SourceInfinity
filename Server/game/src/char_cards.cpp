...
{
	if (GetExchange() || GetMyShop() || GetShopOwner() || IsOpenSafebox() || IsCubeOpen() || IsCombOpen() || isSashOpened(true) || isSashOpened(false) || isChangeLookOpened())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¡Imposible Iniciar el MiniJuego!"));
		return;
	}
	if (character_cards.cards_left <= 0) 
	{
		if (GetGold() < 30000)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¡Yang Insuficiente!"));
			return;
		}
		if (CountSpecifyItem(79506) < 1)
		{
			ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¡Necesitas SetOkey!"));
			return;
		}
...
void CHARACTER::Cards_pullout()
{
	...
	if (empty_space == i)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¡Espacio Insuficiente!"));
		return;
	}
	if (character_cards.cards_left < 1)
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("¡Agotado!"));
		return;
	}
...
{
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT name, SUM(points) as spoints FROM log.okay_event GROUP BY pid ORDER BY spoints DESC LIMIT 10", get_table_postfix()));
...
{
	std::unique_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT name, points FROM log.okay_event ORDER BY points DESC LIMIT 10", get_table_postfix()));
	if (pMsg->Get()->uiNumRows == 0)
	{
		return;
	}
