...
//void LogManager::DungeonRegistros(DWORD dwPlayerID, const char* szPlayerName, DWORD dwPlayerLevel, DWORD dwAtaque, DWORD index)
//{
//	std::auto_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT * FROM player.dungeon_ataque%s WHERE player_id=%u AND dungeon_id=%u;", get_table_postfix(), dwPlayerID, index));
//	if (pMsg->Get()->uiNumRows == 0)
//		Query("INSERT INTO player.dungeon_ataque%s (player_id, player_name, level, ataque, dungeon_id, date) VALUES(%u, '%s', %u, %u, %u, NOW())", get_table_postfix(), dwPlayerID, szPlayerName, dwPlayerLevel, dwAtaque, index);
//	else
//	{
//		DWORD dwPlayerIDR = 0;
//		const char* szPlayerNameR;
//		DWORD dwPlayerLevelR = 0;
//		DWORD dwAtaqueR = 0;
//		DWORD indexR = 0;
//		MYSQL_ROW mRow;
...
//		{
//			int iCur = 0;
//			str_to_number(dwPlayerIDR, mRow[iCur++]);
//			szPlayerNameR = mRow[iCur++];
//			str_to_number(dwPlayerLevelR, mRow[iCur++]);
//			str_to_number(dwAtaqueR, mRow[iCur++]);
//			str_to_number(indexR, mRow[iCur++]);
//		}
//
//		if ((dwAtaqueR > 0))
//			Query("UPDATE player.dungeon_ataque%s SET level=%u, ataque=%u, date=NOW() WHERE player_id=%u AND dungeon_id=%u;", get_table_postfix(), dwPlayerLevel, dwAtaque+dwAtaqueR, dwPlayerID, index);
//	}
//}
//
//void LogManager::DungeonRanking(DWORD dwPlayerID, const char* szPlayerName, DWORD dwPlayerLevel, DWORD dwPunto, DWORD index)
//{
//	std::auto_ptr<SQLMsg> pMsg(DBManager::Instance().DirectQuery("SELECT * FROM player.dungeon_ranking%s WHERE player_id=%u AND dungeon_id=%u;", get_table_postfix(), dwPlayerID, index));
//	if (pMsg->Get()->uiNumRows == 0)
//		Query("INSERT INTO player.dungeon_ranking%s (player_id, player_name, level, finished, dungeon_id, date) VALUES(%u, '%s', %u, %u, %u, NOW())", get_table_postfix(), dwPlayerID, szPlayerName, dwPlayerLevel, dwPunto, index);
//	else
//	{
//		DWORD dwPlayerIDR = 0;
//		const char* szPlayerNameR;
...
//			szPlayerNameR = mRow[iCur++];
//			str_to_number(dwPlayerLevelR, mRow[iCur++]);
//			str_to_number(dwPuntoR, mRow[iCur++]);
//			str_to_number(indexR, mRow[iCur++]);
//		}
//
//		if ((dwPuntoR > 0))
//			Query("UPDATE player.dungeon_ranking%s SET level=%u, finished=%u, date=NOW() WHERE player_id=%u AND dungeon_id=%u;", get_table_postfix(), dwPlayerLevel, dwPunto+dwPuntoR, dwPlayerID, index);
//	}
//}
//
//#endif
