		...
		case QID_AUTH_LOGIN:
			{
				...
				else
				{
					...
					else if (!strcmp(szStatus, "MAC_BAN"))
					{
						LoginFailure(d, "MACBAN");
						sys_log(0, "   MACBAN");
						M2_DELETE(pinfo);
					}
					else if (!strcmp(szStatus, "GUVENLIPC"))
					{
						LoginFailure(d, "GUVENLIPC");
						sys_log(0, "   GUVENLIPC");
						M2_DELETE(pinfo);
					}
					else
					{
						...
						//char dwGuvenli[255 + 1];
						//DBManager::instance().EscapeString(dwGuvenli, sizeof(dwGuvenli), pinfo->guvenlimi, strlen(pinfo->guvenlimi));
						...
						std::unique_ptr<SQLMsg> _hwid_msg2(DBManager::instance().DirectQuery("SELECT hwid FROM account.maclistesi where login='%s' and acik = 1", pinfo->login));
						bool var3 = false;
						if (!_hwid_msg2->uiSQLErrno)
						{
							if (_hwid_msg2->Get()->uiNumRows)
							{
								while (MYSQL_ROW row_hwid = mysql_fetch_row(_hwid_msg2->Get()->pSQLResult))
								{
						...
						bool var2 = false;
						if (pinfo->guvenlimi == 1)
						{
							std::unique_ptr<SQLMsg> _hwid_msg22(DBManager::instance().DirectQuery("SELECT login FROM account.maclistesi where login='%s' and hwid ='%s'", pinfo->login, pinfo->cHWInfo));
							if (!_hwid_msg22->uiSQLErrno)
							{
								if (_hwid_msg22->Get()->uiNumRows)
								{
									var2 = true;
									char pUpdateHwid[1024];
									snprintf(pUpdateHwid, sizeof(pUpdateHwid), "UPDATE account.maclistesi SET hwid = '%s', acik=%d WHERE login = '%s'", pinfo->cHWInfo, pinfo->guvenlimi, pinfo->login);
									std::unique_ptr<SQLMsg> sUpdate(DBManager::instance().DirectQuery(pUpdateHwid));
								}
							}
						}
						if (!var2)
						{
							std::unique_ptr<SQLMsg> _hwid_msg222(DBManager::instance().DirectQuery("SELECT login FROM account.maclistesi where login='%s' and hwid ='%s'", pinfo->login, pinfo->cHWInfo));
							if (!_hwid_msg222->uiSQLErrno)
							{
								if (!_hwid_msg222->Get()->uiNumRows)
								{
									std::unique_ptr<SQLMsg> msg(DBManager::instance().DirectQuery("INSERT IGNORE INTO account.maclistesi SET login='%s', hwid='%s', acik=%d", pinfo->login, szHwid, pinfo->guvenlimi));
		...
		case QID_ACCOUNT_LOCK_SIZE:
		{
			...
			if (ch)
			{
				if (pMsg->Get()->uiNumRows > 0)
				{
					...
					int	size = 0;
					str_to_number(size, row[0]);
					ch->SetAccountLockSize(size);
				}
			}
		}
		break;
		case QID_ACCOUNT_LOCK_PASSWORD:
		{
			LPCHARACTER ch = CHARACTER_MANAGER::instance().FindByPID(qi->dwIdent);
			...
			if (ch)
			{
				if (pMsg->Get()->uiNumRows > 0)
				{
					MYSQL_ROW row = mysql_fetch_row(pMsg->Get()->pSQLResult);
					int	password = 0;
					str_to_number(password, row[0]);
					ch->SetAccountLockPassword(password);
				}
			}
		}
		break;
