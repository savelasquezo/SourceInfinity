...
void CShopManager::Buy(LPCHARACTER ch, BYTE pos)
{
	...
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Hesabin kilitliyken esya satin alamazsin! ");
		return;
	}
#endif
...
void CShopManager::Sell(LPCHARACTER ch, UINT bCell, BYTE bCount)
{
	...
	if (ch->GetAccountLockSize() == 1)
	{
		ch->ChatPacket(CHAT_TYPE_INFO, "Hesap kilitliyken esya satamazsin. ");
		return;
	}
#endif
	...
	if (item->GetVnum() >= 80003 && item->GetVnum() <= 80007 || item->GetVnum() >= 90024 && item->GetVnum() <= 90038)
	{
		if (item->GetCount() != 1)
		{
			ch->ChatPacket(CHAT_TYPE_INFO, "Alt? ve g?淆 barlar??t ?te iken satamazs?.");
			return;
		}
	}
