...
void CSwitchbot::SwitchItems()
{
	for (BYTE bSlot = 0; bSlot < SWITCHBOT_SLOT_COUNT; ++bSlot)
	{
		...
		if (CheckItem(pkItem, bSlot))
		{
			...
			{
				char buf[255];
				int len = snprintf(buf, sizeof(buf), LC_TEXT("Los bonos del item %s (Slot: %d) se han completado, Felicidades!."), pkItem->GetName(), bSlot + 1);
		...
		else
		{
			...
			if (stop)
			{
				...
				if (SWITCHBOT_PRICE_TYPE == 1)
				{
					pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Switchbot> ¡Cambios Insuficientes!"));
				}
				else
				{
					pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("<Switchbot> ¡Yang Insuficiente!"));
				}
