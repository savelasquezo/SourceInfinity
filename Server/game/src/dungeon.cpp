...
namespace
{
	...
	struct FKillSectree
	{
		void operator () (LPENTITY ent)
		{
			...
			{
				LPCHARACTER ch = (LPCHARACTER) ent;
#ifdef ENABLE_GROWTH_PET_SYSTEM && __DEFENSE_WAVE__
				if (!ch->IsPC() && !ch->IsPet() && !ch->IsNewPet() && !ch->IsNewMount() && ch->GetRaceNum() != 20434 && ch->GetRaceNum() != 20436 && !(ch->GetRaceNum() >= 3960 && ch->GetRaceNum() <= 3964))
#else
				if (!ch->IsPC() && !ch->IsPet() )
#endif
					ch->Dead();
