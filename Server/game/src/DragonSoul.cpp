...
bool DSManager::RefreshItemAttributes(LPITEM pDS)
{
	...
	{	
		...
		pDS->SetForceAttributeDragonSoul(i, bType, sValue);
	}
	...
	for (int i = DRAGON_SOUL_ADDITIONAL_ATTR_START_IDX; i < ITEM_ATTRIBUTE_MAX_NUM; i++)
	{
		...
		pDS->SetForceAttributeDragonSoul(i, bType, (short)(ceil((float)sValue * fWeight - 0.01f)));
	}
	return true;
}
...
bool DSManager::PutAttributes2(LPITEM pDS)
{
	...
	if (additional_attr_num > 0)
	{
		...
		for (int i = 0; i < additional_attr_num; i++)
		{
			...
			pDS->SetForceAttributeDragonSoul(DRAGON_SOUL_ADDITIONAL_ATTR_START_IDX + i, bType, sValue);
		}
	}
...
bool DSManager::DoRefineStep(LPCHARACTER ch, TItemPos (&aItemPoses)[DRAGON_SOUL_REFINE_GRID_SIZE])
{
	...
	//{
	//	sys_err ("%s do not activate DragonSoulRefineWindow. But how can he come here?", ch->GetName());
	//	ch->ChatPacket(CHAT_TYPE_INFO, "[Alquimia] No puedes usar la ventana de refinamiento del alma de dragon.");
	//	return false;
	//}
