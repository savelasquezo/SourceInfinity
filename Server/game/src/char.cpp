...
void CHARACTER::PartyInvite(LPCHARACTER pchInvitee)
{
	...
	{
		ChatPacket(CHAT_TYPE_INFO, "Karþ?oyuncunun hesab?kilitli oldu?ndan gruba kat?amaz. ");
		return;
	}
#endif
...
#endif
{
	...
	if (test_server)
		sys_log(0, "%s %p ¨¡¡À¡¾aCI¡Æi ¥ì©ö¨ú¨¡¡Æ¢®AU! %d %d", GetName(), this, x, y);
...
void CHARACTER::SetAccountLockSize(int iSize)
{
	...
	if (GetMyShop())
	{
		ChatPacket(CHAT_TYPE_INFO, "Pazar açýkken hesap kilitleyemezsiniz. ");
		return;
	}

	if (IsCubeOpen())
	{
		ChatPacket(CHAT_TYPE_INFO, "Cube penceresi açýkken hesap kilitleyemezsiniz. ");
		return;
	}

	if (IsOpenSafebox())
	{
		ChatPacket(CHAT_TYPE_INFO, "Depo penceresi açýkken hesap kilitleyemezsiniz. ");
		return;
	}
