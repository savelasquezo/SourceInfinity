...
void CHARACTER::SkillLevelUp(DWORD dwVnum, BYTE bMethod)
{
	...
	//if (pkSk->preSkillVnum)--FIX LEVELUP HABILIDADES SKILL M * 4 Nov 2020
...
struct FuncSplashDamage
{
	...
	void operator () (LPENTITY ent)
	{
			...
			case SKILL_ATTR_TYPE_MELEE:
				{
					...
					if (pkWeapon)
						switch (pkWeapon->GetSubType())
						{
							...
							case WEAPON_SWORD:
								iDam = iDam * (100 - pkChrVictim->GetPoint(POINT_RESIST_HUMAN)) / 100;
		...
		if (pkChrVictim->IsNPC())
		{
			...
			iDam = iDam * (100 - pkChrVictim->GetPoint(POINT_RESIST_MAGIC)) / 10;
			if (pkChrVictim->GetMobRank() == MOB_RANK_BOSS)
			{
				...
				{
					if (m_pkChr->GetWear(WEAR_WEAPON)->GetSubType() == WEAPON_DAGGER)
					{
						iDam *= 1.0;// 00% +DMG
					}
					if (m_pkChr->GetWear(WEAR_WEAPON)->GetSubType() == WEAPON_BOW)
					{
						iDam *= 1.7;// 70% +DMG
					}
					else
			...
			if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_ICE))
			{
				iDam = iDam * (100 - pkChrVictim->GetPoint(POINT_RESIST_ICE)) / 100;
			}
			if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_EARTH))
			{
				iDam = iDam * (100 - pkChrVictim->GetPoint(POINT_RESIST_EARTH)) / 100;
			}
			if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_DARK))
			{
		...
		{
			int iPct = (int) m_pkSk->kPointPoly2.Eval();
			m_pkChr->PointChange(POINT_HP, -abs(iDam * iPct / 100));//FIX POSITIVE DAMAGE
		}

		if (IS_SET(m_pkSk->dwFlag, SKILL_FLAG_SP_ABSORB))
		{
			int iPct = (int) m_pkSk->kPointPoly2.Eval();
			m_pkChr->PointChange(POINT_SP, -abs(iDam * iPct / 100));//FIX POSITIVE DAMAGE
		}
...
{
#ifdef ENABLE_BUFFIX_SKILL_MANA
	...
	{
		ChatPacket(CHAT_TYPE_NOTICE, "<<Hata>> Kullanmak ?ere oldu?n skillIndex %d yetenekleri silah olmadan kullanamazs?? !.", dwVnum);
		return false;
	}
#endif
...
#include "questmanager.h"
	...
	if (dwFlagBlockSkill > 0)
	{
		if (GetMapIndex() == 113)
		{
			if (quest::CQuestManager::instance().GetEventFlag("EventOxEventQz") == 2)
			{
				return false;
				ChatPacket(CHAT_TYPE_NOTICE, "[OXEVENT] No puedo usar habilidades en la sesion del evento.!");			
			}		
		}
	}
#endif
	...
	if (IsActivateSecurity())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GUVENLIK_KILIDI_AKTIFKEN_ATAK_YAPAMASSIN"));
		return false;
	}

	if (pkVictim && pkVictim->IsPC() && pkVictim->IsActivateSecurity())
	{
		ChatPacket(CHAT_TYPE_INFO, LC_TEXT("GUVENLIK_KILIDI_ACIK_OLAN_KARAKTERE_ATAK_YAPILAMAZ"));
		return false;
	}
	#endif
...
bool CHARACTER::IsUsableSkillMotion(DWORD dwMotionIndex) const
{
	...
	static DWORD s_anSkill2JobGroup[SKILL_NUM] = {
		...
		51, // job_skill(WOLFMAN SKILL)	176
