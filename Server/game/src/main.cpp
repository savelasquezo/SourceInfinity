...
void heartbeat(LPHEART ht, int pulse)
{
	...
	if (!(pulse % (passes_per_sec * AUTOBACKUP_SAVE_TIME)))  //*  auto yedek al?//
	{
		if (g_bAuthServer)
	...
	if (!(pulse % (passes_per_sec * AUTOBACKUPDEL_SAVE_TIME))) //*  auto al?an yedekleri siler 48 saatte bir// 3600= 1saat 3600*48= 172800 saniye
	{
		if (g_bAuthServer)
...
int main(int argc, char **argv)
{
	...
	while (!exppet_table_open.eof())
	{
		...
		if (exppet_table_common[exppet_table_counter] < 2147483647) {
			...
			fprintf(stderr, "[Uyari] exppettable.txt dosya bulunmuyor veya dizin yolu yanlis\n");
			break;
		}
	}
#endif
