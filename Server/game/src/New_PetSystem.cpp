...
void CNewPetActor::ItemCubeFeed(int type)
{
	for (int i = 0; i < 9; ++i) {
		if (m_dwpetslotitem[i] != -1) {
			...
			if (itemxp->GetID() == ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID())->GetID() || m_pkOwner->GetExchange() || m_pkOwner->GetMyShop() || m_pkOwner->GetShopOwner() || m_pkOwner->IsOpenSafebox() || m_pkOwner->IsCubeOpen() || m_pkOwner->GetOfflineShopOwner()  || m_pkOwner->IsCombOpen())
				return;
			if(type == 1)
			{
				if (itemxp->GetVnum() >= 55401 && itemxp->GetVnum() <= 55415 || itemxp->GetVnum() >= 55701 && itemxp->GetVnum() <= 55715 || itemxp->GetVnum() == 55001)
				{
			...
			else if(type == 3)
			{
				if(itemxp->GetType() == 1 || itemxp->GetType() == 2)
				{
					if (GetLevel() < 155)
					{
						...
						{
							m_pkOwner->ChatPacket(CHAT_TYPE_INFO, LC_TEXT("PET_DOYDU"));
						}
					}
					else
					{
...
void CNewPetActor:: IncreasePetBonus() {
	...
	if (pSummonItem != NULL){
		for (int b = 0; b < 3; b++){
			pSummonItem->SetForceAttributeNewPets(b, 1, m_dwbonuspet[b][1]);
		}
...
void CNewPetActor::UpdateTime()
{
	...
	if (m_dwTimePet >= 60) {
		...
		LPITEM pSummonItem = ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID());
		if (pSummonItem != NULL){			
			pSummonItem->SetForceAttributeNewPets(3, 1, m_dwduration);
			pSummonItem->SetForceAttributeNewPets(4, 1, m_dwtduration);
		}
		m_pkOwner->ChatPacket(CHAT_TYPE_COMMAND, "PetDuration %d %d", m_dwduration, m_dwtduration);		
	}
...
/*void CNewPetActor::UpdateTime()
{
	...
	if (pSummonItem != NULL)
	{
		pSummonItem->SetForceAttributeNewPets(3, 1, m_dwduration);
		pSummonItem->SetForceAttributeNewPets(4, 1, m_dwtduration);
		pSummonItem->SetForceAttributeNewPets(6, 1, m_dwevolution);
		//lvl guncelleme
...
void CNewPetActor::Unsummon()
{
	if (true == this->IsSummoned())
	{
		...
		if (pSummonItem != NULL){
			...
			pSummonItem->SetForceAttributeNewPets(4, 1, m_dwtduration);
			pSummonItem->SetForceAttributeNewPets(6, 1, m_dwevolution);
			pSummonItem->SetForceAttributeNewPets(7, 1, m_dwskillslot[0]);
			pSummonItem->SetForceAttributeNewPets(8, 1, m_dwskill[0]);
			pSummonItem->SetForceAttributeNewPets(9, 1, m_dwskillslot[1]);
			pSummonItem->SetForceAttributeNewPets(10, 1, m_dwskill[1]);
			pSummonItem->SetForceAttributeNewPets(11, 1, m_dwskillslot[2]);
			pSummonItem->SetForceAttributeNewPets(12, 1, m_dwskill[2]);
			pSummonItem->SetSocket(1,m_dwlevel);
...
bool CNewPetActor::Update(DWORD deltaTime)
{
	...
	if (IsSummoned() && (m_pkOwner->GetExchange() || m_pkOwner->GetShop() || m_pkOwner->GetMyShop() || m_pkOwner->GetShopOwner() || m_pkOwner->IsOpenSafebox() || m_pkOwner->IsCubeOpen() || m_pkOwner->IsCombOpen() || m_pkOwner->GetOfflineShopOwner() || m_dwduration <= 0)
		|| NULL == ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID())
		|| ITEM_MANAGER::instance().FindByVID(this->GetSummonItemVID())->GetOwner() != this->GetOwner()
		)
	{
		m_pkOwner->SetQuestFlag("new_pet_system.pet_vnum", 0);
        m_pkOwner->SetQuestFlag("new_pet_item_id.pet_vnum", 0);
		this->Unsummon();
		return true;
	}
