...
void CInputAuth::Login(LPDESC d, const char * c_pData)
{
	...
	char query[1024];
	snprintf(query, sizeof(query), "SELECT IpAdresi FROM account.IP_Ban WHERE IpAdresi = '%s'", inet_ntoa(d->GetAddr().sin_addr));
	...
	char query2[1024];
	snprintf(query2, sizeof(query2), "SELECT hwid FROM account.yasaklimac WHERE hwid = '%s'", pinfo->cHWInfo);
	std::unique_ptr<SQLMsg> execquery2(DBManager::instance().DirectQuery(query2));

	if (execquery2->Get()->uiNumRows >= 1)
	{
