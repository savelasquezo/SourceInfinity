...
void CGuild::LoncaIstatistik(LPCHARACTER ch)
{
	int member = 0;
		for (itertype(m_memberOnline) it = m_memberOnline.begin(); it!=m_memberOnline.end();++it)
		{
			...
			durum = 0;
			int oldurme = tch->GetQuestFlag("loncaistatistik.oldurme");
			int olum = tch->GetQuestFlag("loncaistatistik.olum");
			ch->ChatPacket(CHAT_TYPE_COMMAND, "loncaistatistik %s %d %d %d %d %d",tch->GetName(),tch->GetLevel(),durum,oldurme,olum,member);
			}
		}
}
